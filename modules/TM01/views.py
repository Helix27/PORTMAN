...
bp = Blueprint('TM01', __name__, template_folder='.')
MODULE_CODE = 'TM01'
...
@bp.route('/module/TM01/')
@login_required
def view():
    perms = get_perms()
    if not perms.get('can_read'):
        return render_template('no_access.html'), 403
    return render_template('tm01.html', permissions=perms)


@bp.route('/api/module/TM01/data')
@login_required
def get_data():
...
@bp.route('/api/module/TM01/save', methods=['POST'])
@login_required
def save():
    perms = get_perms()
    data = request.json
...
@bp.route('/api/module/TM01/delete', methods=['POST'])
@login_required
def delete():
    perms = get_perms()
