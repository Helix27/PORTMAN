...
bp = Blueprint('VIEM01', __name__, template_folder='.')
MODULE_CODE = 'VIEM01'
...
@bp.route('/module/VIEM01/')
@login_required
def view():
    perms = get_perms()
    if not perms.get('can_read'):
        return render_template('no_access.html'), 403
    return render_template('viem01.html', permissions=perms)

@bp.route('/api/module/VIEM01/data')
@login_required
def get_data():
...
@bp.route('/api/module/VIEM01/all')
@login_required
def get_all():
    return jsonify(model.get_all())

@bp.route('/api/module/VIEM01/save', methods=['POST'])
@login_required
def save():
    perms = get_perms()
    data = request.json
...
@bp.route('/api/module/VIEM01/delete', methods=['POST'])
@login_required
def delete():
    perms = get_perms()
...
PARTY_TYPE = 'ImporterExporter'

@bp.route('/api/module/VIEM01/virtual-accounts/<int:party_id>')
@login_required
def get_va(party_id):
    return jsonify(va_utils.get_va_list(PARTY_TYPE, party_id))

@bp.route('/api/module/VIEM01/virtual-accounts/save', methods=['POST'])
@login_required
def save_va():
    perms = get_perms()
...
@bp.route('/api/module/VIEM01/virtual-accounts/delete', methods=['POST'])
@login_required
def delete_va():
    perms = get_perms()
    if not perms.get('can_delete'):
