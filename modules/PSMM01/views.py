...
bp = Blueprint('PSMM01', __name__, template_folder='.')
MODULE_CODE = 'PSMM01'
...
@bp.route('/module/PSMM01/')
@login_required
def view():
    perms = get_perms()
    if not perms.get('can_read'):
        return render_template('no_access.html'), 403
    return render_template('psmm01.html', permissions=perms)

@bp.route('/api/module/PSMM01/data')
@login_required
def get_data():
...
@bp.route('/api/module/PSMM01/all')
@login_required
def get_all():
    return jsonify(model.get_all())

@bp.route('/api/module/PSMM01/save', methods=['POST'])
@login_required
def save():
    perms = get_perms()
    data = request.json
...
@bp.route('/api/module/PSMM01/delete', methods=['POST'])
@login_required
def delete():
    perms = get_perms()
