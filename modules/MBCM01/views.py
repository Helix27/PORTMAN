...
bp = Blueprint('MBCM01', __name__, template_folder='.')
MODULE_CODE = 'MBCM01'
...
@bp.route('/module/MBCM01/')
@login_required
def view():
    perms = get_perms()
    if not perms.get('can_read'):
        return render_template('no_access.html'), 403
    return render_template('mbcm01.html', permissions=perms)

@bp.route('/api/module/MBCM01/data')
@login_required
def get_data():
    rows = model.get_all()
    return jsonify({'data': rows, 'last_page': 1, 'total': len(rows)})

@bp.route('/api/module/MBCM01/all')
@login_required
def get_all():
    rows = model.get_all()
    return jsonify([r['mbc_name'] for r in rows])

@bp.route('/api/module/MBCM01/save', methods=['POST'])
@login_required
def save():
    perms = get_perms()
    data = request.json
...
@bp.route('/api/module/MBCM01/delete', methods=['POST'])
@login_required
def delete():
    perms = get_perms()
