...
def save_data(data):
    ...
    if row_id:
        cur.execute(f"UPDATE {TABLE} SET name=%s, make=%s WHERE id=%s", [name, make, row_id])
    else:
        cur.execute(f"INSERT INTO {TABLE} (name, make) VALUES (%s, %s) RETURNING id", [name, make])
